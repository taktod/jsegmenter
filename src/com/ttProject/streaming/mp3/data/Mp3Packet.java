...
import com.ttProject.streaming.data.MediaPacket;
...
public abstract class Mp3Packet extends MediaPacket {
	...
	private int mpegVersion;    // 0:mpeg1 1:mpeg2 2:mpeg2.5
	private int layer;          // 0:layer1 1:layer2 2:layer3
	private int bitrate;        // kbps表示
	private float samplingRate; // kHz表示
	private int paddingBit;     // 1 or 0
	@SuppressWarnings("unused")
	private int channelMode;    // 0:stereo 1:joint stereo 2:Dual channel 3:mono
	private int startPos = -1;
	private Mp3PacketManager manager;
		...
		while(buffer.remaining() >= 4) {
			...
			if(targetLength != 0) {
				...
				&& readData[2] == '3') {
					...
					if(passedTime >= Segmenter.getDuration()) { // 経過時間が5秒すぎている場合は、次のパケットにすすむ(分割を実行する。)
